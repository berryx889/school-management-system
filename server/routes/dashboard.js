...
router.get('/admin', requireAuth, requireRole('admin'), async (_req, res) => {
  ...
  // All request queries share one RLS-scoped pg client. Run them sequentially; issuing
  // Promise.all against one client is deprecated in pg and can interleave transactions.
  const students = await pool.query("SELECT COUNT(*) FROM students WHERE status='active'");
  const teachers = await pool.query("SELECT COUNT(*) FROM users WHERE role='teacher' AND is_active=true");
  ...
  const feesOutstanding = await pool.query(
  ...
  const recentAnnouncements = await pool.query(`SELECT * FROM announcements ORDER BY created_at DESC LIMIT 5`);
  ...
  const owingStudents = await pool.query(
...
router.get('/super-admin', requireAuth, requireRole('super_admin'), async (_req, res) => {
  const staffRoles = [['super_admin', 'admin', 'teacher', 'kitchen', 'accountant']];
  const staff = await pool.query("SELECT COUNT(*) FROM users WHERE role = ANY($1::user_role[]) AND deleted_at IS NULL", staffRoles);
  const inactiveStaff = await pool.query("SELECT COUNT(*) FROM users WHERE role = ANY($1::user_role[]) AND is_active=false AND deleted_at IS NULL", staffRoles);
  const auditToday = await pool.query("SELECT COUNT(*) FROM audit_logs WHERE created_at >= CURRENT_DATE");
  const failedLogins = await pool.query("SELECT COUNT(*) FROM audit_logs WHERE action='auth.login_failed' AND created_at >= now() - INTERVAL '24 hours'");
  const activeClasses = await pool.query("SELECT COUNT(*) FROM classes WHERE deleted_at IS NULL");
  const integrations = await pool.query(`SELECT
